...
def analyze_patterns(examples: List[Dict[str, str]]) -> Dict:
    ...
    for ex in examples:
        ...
        if where_match:
            ...
            column_matches = re.findall(r'(\w+)\s*(?:=|>|<|>=|<=|!=|LIKE|IN)', 
        ...
        comparison_keywords = ['compare', 'differ', 'difference', 'versus', 'vs', 
                              'between', 'higher than', 'lower than', 'greater than']
...
def create_prompt_for_generation(examples: List[Dict[str, str]], 
...
def validate_queries_batch(examples: List[Dict[str, str]], db_path: str) -> List[Dict[str, str]]:
        ...
        for i, ex in enumerate(examples):
            ...
            try:
                ...
                if has_results:
                    ...
                    print(f"  Query {i+1} returned no results: {ex['question'][:50]}...")
...
def generate_dataset(existing_csv: str, 
                     db_path: str, 
                     output_csv: str, 
                     target_count: int):
    ...
    for batch_num in range(batches_needed):
        ...
        prompt = create_prompt_for_generation(existing_example_dict, schema, patterns, batch_size=10)
        ...
        print(f"  Added {len(valid_examples)} valid examples ({len(valid_examples)}/{len(new_examples)} success rate)")
    ...
    print(f"✓ Done! Generated {len(all_examples) - len(existing_example_dict)} new examples")
...
if __name__ == "__main__":
    ...
    print_pattern_summary(
        analyze_patterns(
            load_existing_examples(output_csv_path)
        )
    )

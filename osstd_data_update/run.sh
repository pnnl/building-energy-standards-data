...
echo "Step 6: push new branch to OSSTD github (currently updating exterior lighting requirements (**/*ext_lgt*.json) data only)"
git config --global user.email "xuechen.lei@pnnl.gov"
git config --global user.name "Xuechen (Jerry) Lei"
git add **/*ext_lgt*.json
...
git commit -m "data_update (ext_lgt_only) ${COMMIT_ID:0:7}"
